...
function HandBookList({state}) {
  return (
    <div className="container my-24 px-6 p-3 mx-auto">
    ...
    <section className="mb-32 text-gray-800">
        <div className='pb-20'>
            ...
            <p className={`mb-15 font-normal text-center text-gray-500 text-4xl px-12 sm:px-16 xl:pl-5 dark:text-gray-400 ${state.langMode ? "shown" : "hidden"}`}>行前注意</p>
        </div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3 pb-20">
            <a href="/phone" rel="noreferrer">
            <div className="zoom-box flex flex-col rounded-xl bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 md:max-w-xl md:flex-row">
                <img
                    className="h-96 w-full rounded-t-xl object-cover md:h-auto md:w-48 md:rounded-none md:rounded-l-xl"
                    src={telecoms}
                    alt="" />
                <div className="flex flex-col justify-start p-6">
                    <h5 className={`mb-2 text-xl font-medium text-neutral-800 dark:text-neutral-800 ${state.langMode ? "shown" : "hidden"}`}>
                        手機資訊
                    </h5>
                    <h5 className={`mb-2 text-xl font-medium text-neutral-800 dark:text-neutral-800 ${state.langMode ? "hidden" : "shown"}`}>
                        Phone Information
                    </h5>
                    <p className={`mb-4 text-base text-neutral-600 dark:text-neutral-800 ${state.langMode ? "shown" : "hidden"}`}>
                        電話信息是指與移動電話或固定電話號碼相關的個人數據和使用詳細信息，包括通話記錄、短信歷史記錄和其他相關元數據。
                    </p>
                    <p className={`mb-4 text-base text-neutral-600 dark:text-neutral-800 ${state.langMode ? "hidden" : "shown"}`}>
